...
const isInitialized = (top, left, scale) =>
...
export default class PinchZoomPan extends React.Component {
  ...
  zoomIn(midpoint, speed = 0, factor = 0.1) {
    if (this.state.scale === 1 && this.state.scale * (1 + factor) > 1) {
  ...
  zoomOut(midpoint) {
    ...
    if (this.state.scale > 1 && this.state.scale * 0.9 <= 1) {
      this.props.onZoomEnd();
    }
  ...
  zoom(requestedScale, containerRelativePoint, tolerance, speed = 0) {
    ...
    // Disables Overzoom
    // Overzoom sometimes causes the image to shift after letting go of the 2 finger pinch
    if (this.props.disableOverzoom && nextScale > this.props.maxScale) {
      nextScale = this.props.maxScale;
    }
    ...
    if (scale > 1 && requestedScale <= 1) {
      this.props.onZoomEnd();
    } else if (scale === 1 && requestedScale > 1) {
      this.props.onZoomStart();
    }
  ...
  applyInitialTransform(speed = 0) {
    const { imageDimensions, containerDimensions } = this.state;
    const { position, initialScale, maxScale, initialTop, initialLeft } =
      this.props;
...
PinchZoomPan.propTypes = {
  ...
  disableOverzoom: PropTypes.bool,
};
